...
class TestPoolTransaction:
    ...
    def setup_class(cls):
        ...
        # Setup pools for different databases
        cls.pools = {}
        
        # PostgreSQL pool
        try:
            pg_cfg = cls.cfg.require("postgres-test")
            cls.pools['postgres'] = Pool.from_config(pg_cfg, PoolBackend.DBUTILS)
            logger.info("Created PostgreSQL pool")
        except Exception as e:
            logger.warning(f"Could not create PostgreSQL pool: {e}")
        
        # MySQL pool
        try:
            mysql_cfg = cls.cfg.require("mysql-test")
            cls.pools['mysql'] = Pool.from_config(mysql_cfg, PoolBackend.DBUTILS)
        ...
        for db_type, pool in self.pools.items():
            try:
                with pool.client() as client:
                    ...
                    logger.info(f"Setup test table for {db_type}")
            except Exception as e:
                logger.error(f"Error setting up test table for {db_type}: {e}")
        ...
        for db_type, pool in self.pools.items():
            try:
                with pool.client() as client:
                    db = Db(client)
                    db.execute("DROP TABLE IF EXISTS test_transaction", [])
                    logger.info(f"Cleaned up test table for {db_type}")
            except Exception as e:
                logger.error(f"Error cleaning up test table for {db_type}: {e}")
    
    def test_transaction_commit_success(self):
        """Test that successful transaction commits all changes"""
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing transaction commit for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                with pool.transaction() as tx:
                    ...
                    logger.info(f"Inserted 2 rows in transaction for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Transaction committed successfully for {db_type}")
                    logger.info(f"  Verified {result['count']} rows persisted")
            
            except Exception as e:
                logger.error(f"✗ Transaction commit test failed for {db_type}: {e}")
                raise
    ...
    def test_transaction_rollback_on_error(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing transaction rollback for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                try:
                    with pool.transaction() as tx:
                        ...
                        # Insert valid row
                        db.insert('test_transaction', {'name': 'row1', 'value': 100, 'description': 'First row'})
                        logger.info(f"Inserted first row for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Transaction rolled back successfully for {db_type}")
                    logger.info(f"  Verified 0 rows persisted (rollback worked)")
            
            except AssertionError:
                raise
            except Exception as e:
                logger.error(f"✗ Transaction rollback test failed for {db_type}: {e}")
                raise
    ...
    def test_transaction_manual_commit(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing manual commit for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                with pool.transaction() as tx:
                    ...
                    # Manual commit
                    tx.commit()
                    logger.info(f"Manually committed transaction for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Manual commit worked for {db_type}")
                    logger.info(f"  Verified {result['count']} rows persisted")
            
            except Exception as e:
                logger.error(f"✗ Manual commit test failed for {db_type}: {e}")
                raise
    ...
    def test_transaction_manual_rollback(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing manual rollback for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                with pool.transaction() as tx:
                    ...
                    # Manual rollback
                    tx.rollback()
                    logger.info(f"Manually rolled back transaction for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Manual rollback worked for {db_type}")
                    logger.info(f"  First insert rolled back, second persisted")
            
            except Exception as e:
                logger.error(f"✗ Manual rollback test failed for {db_type}: {e}")
                raise
    ...
    def test_transaction_with_isolation_level(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing isolation level for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                with pool.transaction(isolation_level="SERIALIZABLE") as tx:
                    ...
                    logger.info(f"Transaction with SERIALIZABLE isolation for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Isolation level test passed for {db_type}")
            
            except Exception as e:
                # Some databases might not support certain isolation levels
                logger.warning(f"Isolation level test skipped for {db_type}: {e}")
    
    def test_multiple_operations_in_transaction(self):
        """Test multiple different operations in one transaction"""
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing multiple operations for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                with pool.transaction() as tx:
                    ...
                    logger.info(f"Performed INSERT, UPDATE, SELECT in transaction for {db_type}")
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Multiple operations committed successfully for {db_type}")
            
            except Exception as e:
                logger.error(f"✗ Multiple operations test failed for {db_type}: {e}")
                raise
    ...
    def test_connection_state_after_transaction(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing connection state restoration for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                with pool.client() as client:
                    ...
                    logger.info(f"✓ Connection state restored properly for {db_type}")
            
            except Exception as e:
                logger.error(f"✗ Connection state test failed for {db_type}: {e}")
                raise
    ...
    def test_transaction_context_manager_interface(self):
        ...
        for db_type, pool in self.pools.items():
            logger.info(f"\n{'='*80}")
            logger.info(f"Testing transaction context interface for {db_type}")
            logger.info(f"{'='*80}")
            ...
            try:
                with pool.transaction() as tx:
                    ...
                    logger.info(f"✓ Transaction context interface is correct for {db_type}")
            
            except Exception as e:
                logger.error(f"✗ Interface test failed for {db_type}: {e}")
                raise
